...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="preamble">
		<h2>Sets of Confirmed T-DNA Insertion Lines</h2>
		<p>Sorted by stock number</p>
		<p>
			Displaying
			<xsl:value-of select="/SetOfLinesCatalogPage/@floor" />
			-
			<xsl:value-of select="/SetOfLinesCatalogPage/@ceiling" />
			of
			<xsl:value-of select="/SetOfLinesCatalogPage/@resultSize" />
			in category
		</p>
	</xsl:template>
	...
	<xsl:template name="main">
		<xsl:for-each select="SetOfLinesCatalogPage/stocks/stock">
			<tr valign="top">
