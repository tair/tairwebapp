...
<%@ page import="org.tair.search.*, org.tair.utilities.*, org.tair.querytools.*, org.tair.abrc.stock.*, java.util.*" %>

<%!
String pageName = "Ecotype Search Results";
%>
...
EcotypeSummary[] results = (EcotypeSummary[]) searchCollection.getResults();
...
<table width="100%" align="center" border="0" cellspacing="0" cellpadding="2">
	...
	<td width="100%"> 
          ...
          <h1>TAIR Ecotype Search Results</h1>
	  ...
	  <tr>
	    ...
	    <td align="right" colspan="3"><input type="button" value="Order Checked Stock" onClick="mySubmit( 0, 'order', 25 )"></td>
          ...
          <tr align="left" <%=myBgColor.toggleBgColor()%> valign="bottom"> 
            ...
            <th class="sm">Name</th>
            <th class="sm">Abbrev<br>Name</th>
            <th class="sm">Alias</th>
            <th class="sm">Location</th>
            <th class="sm">Country</th>
            <th class="sm" align="center">Stock<br>Number</th>
	...
	<% for ( int i = 0; i < results.length; i++ ) { %>
	  ...
	  <tr <%= color %> valign="top">
      	    ...
      	    <td class="sm"><%= floor++ %> <input type="checkbox" name="id" value="<%= results[ i ].getSpeciesVariantID() %>"></td>
            <td class="sm"><%= EmptyTextConverter.convert( results[ i ].getOriginalName() ) %>&nbsp;</td>
            <td class="sm"><a href="/servlets/TairObject?id=<%= results[ i ].getSpeciesVariantID() %>&type=species_variant"><%= EmptyTextConverter.convert( results[ i ].getAbbrevName() ) %>&nbsp;</a></td>
            <td class="sm"><%= EmptyTextConverter.convert( results[ i ].getAliasesString() )%>&nbsp;</td>
            <td class="sm"><%= EmptyTextConverter.convert( results[ i ].getLocation() ) %>&nbsp;</td>
            <td class="sm"><%= EmptyTextConverter.convert( results[ i ].getCountry() ) %>&nbsp;</td>
  
             <!-- only show order checkbox if associated germplasm has a stock that is available -->
            <% if ( results[ i ].hasStocks() ) { %>
                 ...
                 %>
                   ...
                   <% } %>
                       ...
                       <td class="sm" align="center"><a href="/servlets/TairObject?type=stock&id=<%= stock.get_stock_id() %>"><%= stock.get_name() %></a></td>
