...
String sortColumn = null; // need to set these differently depending on origin of search
String query_id = (String) request.getAttribute( "query_id" );
//String groupColumn = null;
if ( origin.equalsIgnoreCase( "publication_detail" ) ) {
    sortColumn = "keyword_category";
    // groupColumn = "tair_object_id";
}  
ResultsFetcher results = new ResultsFetcher( sessionID, summaryType );
results.setPageSize( pageSize );
if ( currentPage == 1 ) { // invoke sorting/grouping only before display of first page
    results.setSort( sortColumn );
    //results.setGroup( groupColumn );
}
...
scrollLinks = ScrollCalculator.getTableScrollLinks( pageSize, currentPage, results.getTotPages() );
}
%>
