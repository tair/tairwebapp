...
<input type="hidden" name="dna_type" value="vector" />
...
<table width="100%" align="center" border="0" cellspacing="0" cellpadding="2">
	...
	<td width="100%"> 
          ...
          <h1>TAIR Vector Search Results</h1>
	...
	<% for ( int i = 0; i < results.length; i++ ) { %>
	  ...
	  <tr <%= bgColor %> valign="top">
            <td class="sm" align="left"><%= floor++ %></td>
      	    <td class="sm" align="center"><input type="checkbox" name="id" value="<%= results[ i ].get_vector_id() %>"></td>
            <td class="sm"><a href="/servlet/TairObject?id=<%= results[ i ].get_vector_id() %>&type=vector"><%= results[ i ].get_name() %>&nbsp;</a></td>
            <td class="sm"><%= EmptyTextConverter.convert( results[ i ].get_alias_string() ) %>&nbsp;</td>
     	    <td class="sm"><%= results[ i ].get_type() %></td>

             <!-- only show order checkbox if vector has a stock that is available -->
