...
<input type="hidden" name="dna_type" value="library" />
...
<table width="100%" align="center" border="0" cellspacing="0" cellpadding="2">
	...
	<td width="100%"> 
          ...
          <h1>TAIR Library Search Results</h1>
	...
	<table width="100%" border="0" cellspacing="0">
	  <tr>
	    ...
	    <td align="right" colspan="2"><input type="button" value="Order Checked Stock" onClick="javaScript:mySubmit( 0, 'order', 25 )" ></td>
	  </tr>
	...
	<% for ( int i = 0; i < results.length; i++ ) { %>
	  ...
	  <tr <%= bgColor %> valign="top">
      	    ...
      	    <td class="sm" align="left"><%= floor++ %><input type="checkbox" name="id" value="<%= results[ i ].get_library_id() %>"></td>
            <td class="sm"><a href="/servlet/TairObject?id=<%= results[ i ].get_library_id() %>&type=library"><%= results[ i ].get_name() %>&nbsp;</a></td>
            <td class="sm"><%= EmptyTextConverter.convert( results[ i ].get_description() ) %>&nbsp;</td>
  
             <!-- only show order checkbox if library has a stock that is available -->
              <% if ( results[ i ].get_stocks() != null ) { %>
