...
int pageSize = ( request.getAttribute( "size" ) != null ) ? ( ( Integer )request.getAttribute( "size" ) ).intValue() : 100;
int currentPage = ( request.getAttribute( "pageNum" ) != null ) ? ( ( Integer )request.getAttribute( "pageNum" ) ).intValue() : 1;
String sortColumn = "annotation_go_po_type, keyword_category"; // need to set these differently depending on origin of search  
ResultsFetcher results = new ResultsFetcher( query_id, summaryType );
results.setSort( sortColumn );
results.setPageSize( pageSize );
// we can comment this part out during the oracle-migration because sort is taken care of now
// with results.setSort() method.
//if ( currentPage == 1 ) { // invoke sorting/grouping only before display of first page
//    results.setKeywordSort( sortColumn, "RESULT_ANNOTATION", query_id );
//}
...
[<A href="/help/helppages/annotdetail.jsp" target="_blank">Help</A>]
</td>
</tr>
